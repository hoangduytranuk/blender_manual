...
public class Dropper extends TransferHandler {
    ...
    private void adjustImportTime(int from_row){
        Subtitles subs = parent.getSubtitles();
        
        int len=subs.size();        
        Time a_start_time = null;
        Time a_end_time = null;
        int a_duration = 0;

        Time b_start_time = null;
        Time b_end_time = null;      
        int b_duration = 0;
        ...
        try {
            boolean is_duration_zero, 
                    is_start_time_greater_than_end_time;
            
            int valid_from_row = Math.max(0, Math.min(from_row, len));            
            for (int i = valid_from_row; i < len - 1; i++) {
                
                boolean is_first_row = (i == valid_from_row);
                if (is_first_row) 
                {
                    a_sub = subs.elementAt(i);
                    a_start_time = a_sub.getStartTime();
                    a_end_time = a_sub.getFinishTime();
                    a_duration = a_end_time.getMilli() - a_start_time.getMilli();
                    correctDuration(a_sub, 
                    if (a_duration <)
                }else{
                    b_sub = subs.elementAt(i);
                    b_start_time = b_sub.getStartTime();
                    b_end_time = b_sub.getFinishTime();
                    b_duration = b_end_time.getMilli() - b_start_time.getMilli();
                    
                    boolean is_start_ealier = (b_start_time.getMilli() < a_end_time_};
                    
                a_sub = b_sub;
            }//end for(int i=from_row; i< len-1; i++)        
        } catch (Exception ex) {
            DEBUG.logger.log(Level.WARNING, ex.toString());
        }
    }//end adjustImportTime    
    
    private boolean correctDuration(SubEntry sub, int minimum_duration_milli){
        boolean changed = false;
        Time start_time = sub.getStartTime();
        Time end_time = sub.getFinishTime();
        int duration = end_time.getMilli() - start_time.getMilli();
        boolean is_valid_duration = (duration >= minimum_duration_milli);
        if (! is_valid_duration){
            int end_time_milli = start_time.getMilli() + minimum_duration_milli;
            end_time.setMilli(end_time_milli);
            changed = true;
        }//end if
        return changed;
    }
