...
def outputDocument(doctree, output_path, is_beautify):
    dir_name = os.path.dirname(output_path)
    text = str(doctree)
    try:
        os.makedirs(dir_name, exist_ok=True)
        ...
        print("Exception outputDocument:{}".format(output_path))
        raise e

def writeDocument(doctree, docname, build_dir):
    #output_file="{}.html".format(docname)
    ...
    is_beautify = ('beauty' in build_dir)
    outputDocument(doctree, output_path, is_beautify)
    #pp(text)
    #exit(0)
...
def visit_paragraph_pdf(self, node):
    self.visit_paragraph_html(node)

def depart_paragraph_pdf(self, node):
    self.depart_paragraph_html(node)
...
def writeDocuments(app, doctree, docname, build_dir, beauty_dir):
    control_var = "RECORD_RST_AS_HTML"
    beatify_var = "BEAUTIFY_HTML"
    has_control_var = (control_var in os.environ)
    must_beautify = (beatify_var in os.environ)
    is_record = False
    is_beautify = False
    if (has_control_var):
        is_record = bool(os.environ[control_var])
        is_beautify = bool(os.environ[beatify_var])

    if (is_beautify):
        writeDocument(doctree, docname, beauty_dir)

    if (is_record):
        writeDocument(doctree, docname, build_dir)

    #exit(0)

def process_title(app, doctree, docname):
    beauty_dir="build/rstdoc_beauty"
    ...
    writeDocuments(app, doctree, docname, build_dir, beauty_dir)

def process_title_0001(app, pagename, templatename, context, doctree):
    beauty_dir="build/rstdoc_beauty_html"
    build_dir="build/rstdoc_html"

    print("pagename:{}".format(pagename))
    print("context:{}".format(context))
    print("type(context):{}".format(type(context)))

    writeDocuments(app, doctree, pagename, build_dir, beauty_dir)
...
def setup(app):
    ...
    #app.add_node(paragraph, pdf=(visit_paragraph_html, depart_paragraph_html))
    ...
    #listender_id = app.connect('html-page-context', process_title_0001)
