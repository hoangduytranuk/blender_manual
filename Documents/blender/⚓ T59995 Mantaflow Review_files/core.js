...
anchor_nodes=JX.DOM.scry(document.body,'a');for(ii=0;ii<anchor_nodes.length;ii++){if(!anchor_nodes[ii].name){continue;}if(anchor_nodes[ii].name.substring(0,anchor.length)===anchor){node=anchor_nodes[ii];break;}}}if(target&&(target!==node)){JX.DOM.alterClass(target,'anchor-target',false);target=null;}if(!node){if(retry_ms>0){retry_ms-=wait_ms;setTimeout(try_anchor,wait_ms);return;}}if(target!==node){target=node;JX.DOM.alterClass(target,'anchor-target',true);}try{var
