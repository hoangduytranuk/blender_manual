...
public class Share implements CommonDef {
    ...
    public static boolean hasTrailingFileSeparator(String s){
        try{
            return s.endsWith(FILE_SEP);
        }catch(Exception e){
            return false;
        }
    }
    ...
    public static boolean CopyFile(File src, File dst) throws Exception {
        ...
        try {
            src.getName(); //check for validity
            dst.getName(); //check for validity
            in = new FileInputStream(src);
            out = new FileOutputStream(dst);
            while (in.read(buffer) != -1) {
                out.write(buffer);
            }//end while(in.read(buffer))
            result = true;
        } catch (Exception ex) {
            result = false;
            throw ex;
        } finally {
            if (!Share.isEmpty(out)) {
                out.close();
            }
            if (!Share.isEmpty(in)) {
                in.close();
    ...
    }//end static boolean CopyFile(File src, File dst)
