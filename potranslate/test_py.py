...
" các tham số đầu vào (ví dụ: :abbr:`màu khuếch tán (diffuse color)`)"
        ...
        for m in data:
            ...
            if can_pair:
                ...
                for ((id_loc, mid_txt), (str_loc, mstr_txt)) in pair_list:
                    ...
                    is_ast = (df.SNG_AST_QUOTE.search(mid_txt) is not None)
                    if is_ast:
                        mid_txt = mid_txt[1:-1]
                        (s_s, s_e) = str_loc
                        str_loc = (s_s+1, s_e-1)
    ...
    def fixGlossaryEntriesMakeEnGoesFirst(self):
        ...
        def reworkCurrentTitles(m: Message):
            ...
            has_brackets = (df.ARCH_BRAKET_MULTI_SIMPLE.search(mid) is not None)
            if not has_brackets:
                return None
    ...
    def correctBrackets(self):
        def correctBracketIfhasInternalBrackets(entry):
            ...
            loc: (int, int) = entry[0]
            text_within = entry[1]
            match_dict = pu.patternMatchAll(df.ARCH_BRAKET_MULTI_SIMPLE, text_within)
    ...
    def sudoku(self):
        ...
        def isPossibleCell(entry):
            row = entry[0]
            col = entry[1]
            value = entry[2]
            cell_value = grid[row][col]
            is_taken = (cell_value == value)
            return is_taken

        def isPossibleCol(col: int, value: int):
            row_range = list(range(9))
            entry_list = [(row, col, value) for row in row_range]
            is_taken_list = list(filter(isPossibleCell, entry_list))
            is_taken = any(is_taken_list)
            return is_taken

        def isPossibleRow(row: int, value: int):
            col_range = list(range(9))
            entry_list = [(row, col, value) for col in col_range]
            is_taken_list = list(filter(isPossibleCell, entry_list))
            is_taken = any(is_taken_list)
            return is_taken

        def isPossibleSquare(x: int, y: int, value: int):
            x0 = (x // 3) * 3
            y0 = (y // 3) * 3
            for row in range(0, 3):
                for col in range(0, 3):
                    cel_y = y0 + col
                    cel_x = x0 + row
                    cell_value = grid[cel_x][cel_y]
                    is_taken = (cell_value == value)
                    if is_taken:
                        return [cel_x, cel_y, cell_value]
            return [-1, -1, -1]

        def possible(row: int, col: int, value: int):
            is_possible = True
            is_taken = isPossibleRow(row, value)
            if is_taken:
                return False

            is_taken = isPossibleCol(col, value)
            if is_taken:
                return False

            (x, y, cell_value) = isPossibleSquare(row, col, value)
            is_taken = (x != -1)
            if is_taken:
                cell_value = grid[x][y]
                return False
        ...
        def tryValue(row: int, col: int, value: int):
            changed = False
            cell = grid[row][col]
            is_filled = (cell != 0)
            if not is_filled:
                is_pos = possible(row, col, value)
                if is_pos:
                    grid[row][col] = value
                    changed = (row, col)
            return changed
        ...
        def tryRow(row: int):
            col = tryRow.col
            value = tryRow.value
            changed = tryValue(row, col, value)
            return changed
        ...
        def tryCol(col: int):
            ...
            tryRow.col = col
            tryRow.value = value
            changed_list = list(filter(tryRow, row_range))
            changed = any(changed_list)
            return changed

        def testValue(value: int):
            col_range = list(range(9))
            tryCol.value = value
            changed_list = list(filter(tryCol, col_range))
            changed = any(changed_list)
            if changed:
                cell_value = grid[0][0]
                debug = True
        ...
        def checkValidGrid():
            for row in range(9):
                for col in range(9):
                    has_zero = (grid[row][col] == 0)
                    if has_zero:
                        return False
            return True

        def resetGrid(saved_copy):
            for row in range(9):
                for col in range(9):
                    grid[row][col] = saved_copy[row][col]

        def solve():
            import copy
            from collections import deque
            orig_grid = copy.deepcopy(grid)
            value_range = deque(list(range(1, 10)))
            for i in range(10):
                changed_list = list(map(testValue, value_range))
                is_finished = checkValidGrid()
                if not is_finished:
                    pp(grid)
                    print(f'tried times: {i+1}, value_range:{value_range}')
                    resetGrid(orig_grid)
                    value_range.rotate(1)
                else:
                    pp(grid)
    ...
    def tranRefs(self):
        ...
        def translateRefs(m: Message):
            ...
            def translate_handler(ref_handler: RefBase):
                ...
                translate_handler.global_untran_list.extend(ref_handler.untranslated)
                return ref_handler
    ...
    def tranRefDriver(self):
        ...
        if is_debug:
            ...
            ''', re.VERBOSE)
            # ((?<!\s)[diouxXeEfFgGcrs%])
            t = "0.1 will offset it by 10% of the view width/height"
            m = PYTHON_FORMAT.search(t)
            print(m)
            exit(0)

            t = "PDT Command Line"
            m = df.GA_GO_FIRST.search(t)
            print(m)
            exit(0)
            p = re.compile(r'^[\s\=\.\+\-e\d\*\/x×]+$')
            t = "256×256"
            m = pu.patternMatchAll(p, t)
            print(m)
            exit(0)
            p = re.compile(r'^(([\-]+\w+)(\s[\+\-]?\d+)?\s?)+$')
            t = "-s 10 -e 500"
            t = "-t"
            t = "-con"
            t = "-E help"
            obs = LocationObserver(t)
            m = pu.patternMatchAll(p, t)
            loc_list = list(m.keys())
            obs.markLocListAsUsed(loc_list)
            print(obs.blank)
            print(len(obs.blank.strip('¶ ')))
            print(f'{t}, {len(t)}')
            print(m)
            exit(0)
            t = "interface_splash_current.png, test.py"
            p = re.compile(r"\S+\.\w+")
            m = p.findall(t)
            print(m)
            exit(0)
            t= "The *Normal* node returns a vector for each evaluated point indicating the :ref:`normal direction <modeling-meshes-structure-normals>`. The output can depend on the attribute domain used in the node evaluating the field, but the output is always a normalized `unit vector <https://en.wikipedia.org/wiki/Unit_vector>`__."
            p = re.compile(r'\`[^`<>]+\<[^\<\>]+\>\`(\_{2})?')
            m  = pu.patternMatchAll(p, t)
            print(m)
            exit(0)
            ...
            ast_quote_txt_sng = r'(?<=\s)\*{1}((?![\s\*]).+?)\*{1}(?=\s)'
            # ast_quote_txt_sng_loose = r'(?<!\*)[\*](([^\*]+)|(?!(\*\*))(.+))[\*]+(?!=\*)'
            ast_quote_txt_sng_loose = r'(?<=\s)[\*]([\w\*\s]+?)[\*]+(?=\s)'
            l = [
                ast_quote_txt_dbl,
                ast_quote_txt_sng,
                # ast_quote_txt_sng_loose,
            ]
            temp = []
            for ll in l:
                txt = r'(%s)' % (ll)
            ...
            p_txt = '|'.join(temp)
            p = re.compile(p_txt)
            l = pu.patternMatchAll(p, t)
            for (loc, mm) in l.items():
                print(mm.getSubEntriesAsList())
                print('-'*80)
            exit(0)
            var = r'(?:\w)[\w\d\_\-]+'
            param_txt = r'(?:[^s])[\w\d\.\_]+'
            ...
            ga_multi = r'(\`+)?'
            funct_pat_txt = r'(%s\(%s\))(?<!(\w\(s\)))' % (var, param)
            funct_only_pat_txt = r'^(%s)$' % (funct_pat_txt)
            p = re.compile(funct_pat_txt)

            t = "float(s) and RGBA(1.0, 0.0, 1.0)"
            m = p.findall(t)
            print(m)
            exit(0)
            l1 = [1, 2, 3, 4, 5]
            l2 = [6, 7]
            result = list(map(printList, l1, l2))

            exit(0)
            pat_txt = r'(?<!\:)\`+((?=\w)[^`:]+(?<=\w))\`+(?=[^_])'
            p = re.compile(pat_txt)
            t = "A cartoon scene from `OHA Studio <http://oha-studios.com/>`__ © Mechanimotion Entertainment. (`blend-file <https://download.blender.org/demo/test/FreeStyle_demo_file.blend.zip>`__)"
            l = p.findall(t)
            t1 = "``min``, ``max``, ``radians``, ``degrees``, ``abs``, ``fabs``, ``floor``, ``ceil``, ``trunc``, ``round``, ``int``, ``sin``, ``cos``, ``tan``, ``asin``, ``acos``, ``atan``, ``atan2``, ``exp``, ``log``, ``sqrt``, ``pow``, ``fmod``"
            l1 = p.findall(t1)
            t2 = "``-m \"message\"``and ``--factory-startup --debug-all`` channel in the ``#docs`` or ``#blender-coders`` channel in :ref:`blender-chat`."
            pat_txt = r'(?<!(:))\`+(?:[\-#]+)([^`]+)\`+'
            p = re.compile(pat_txt)
            l2 = p.findall(t2)
            print(l)
...
" các tham số đầu vào (ví dụ: :abbr:`màu khuếch tán (diffuse color)`)"
    ...
    def test_pat(self):
        home = os.environ['HOME']
        # file_path = os.path.join(home, 'Dev/tran/blender_docs/locale/vi/LC_MESSAGES/blender_manual.po')
        ...
        # home = os.environ['HOME']
        # file_path = os.path.join(home, 'Dev/tran/blender_manual_0003_0021.po')
        # out_file = os.path.join(home, 'test.log')
        # data = c.load_po(file_path)
        # out_data=[]
        # for m in data:
        #     mid = m.id
        #     m = df.GA_REF_ABS.search(mid)
        #     if m is not None:
        #         out_data.append(m.group(0))
    ...
    def sudoku(self):
        class LocRecord:
            def __init__(self, value: int, is_temporal=True, row=-1, col=-1):
                self.row = row
                self.col = col
                self.value: int = value
                self.possible_values = []
                self.is_value_temporal: bool = is_temporal
                self.my_row = []
                self.my_col = []
                self.my_square = []
            ...
            def clearMyRowColSquares(self):
                self.my_row.clear()
                self.my_row.clear()
                self.my_square.clear()

            def isInMy(self, value: int):
                row_list = self.getMyRowValues()
                col_list = self.getMyColValues()
                square_list = self.getMySquareValues()
                value_in_row = (value in row_list)
                value_in_col = (value in col_list)
                value_in_square = (value in square_list)

                is_in = (value_in_row or value_in_col or value_in_square)
                return is_in

            def isNotInMy(self, value: int):
                is_in = self.isInMy(value)
                return not is_in

            def hasNakedSingle(self):
                result = list(filter(self.isNotInMy, self.possible_values))
                return any(result)

            def hasNakedPair(self):
                n_values = len(self.possible_values)
                is_two = (n_values == 2)
                if not is_two:
                    return False

                result = list(filter(self.isNotInMy, self.possible_values))
                has_pairs = (result == self.possible_values)
                return has_pairs


            def getMyRowValues(self):
                return [r.value for r in self.my_row]

            def getMyColValues(self):
                return [r.value for r in self.my_col]

            def getMySquareValues(self):
                return [r.value for r in self.my_square]

            def valueInMyRow(self, value: int):
                return (value in self.getMyRowValues())

            def valueInMyCol(self, value: int):
                return (value in self.getMyColValues())

            def valueInMySquare(self, value: int):
                return (value in self.getMySquareValues())

        class GridRecords(list):
            def __init__(self, grid: list):
                self.grid = grid

            def getSquareCoord(self, row: int, col: int):
                sqr_x0 = (row // 3) * 3
                sqr_y0 = (col // 3) * 3
                return (sqr_x0, sqr_y0)

            def genPossibleValues(self, row: int, col: int):
                r: LocRecord = self[row][col]
                for value in range(1, 10):
                    value_in_my_row = r.valueInMyRow(value)
                    value_in_my_col = r.valueInMyCol(value)
                    value_in_my_square = r.valueInMySquare(value)
                    is_possible = not (value_in_my_row or value_in_my_col or value_in_my_square)
                    if is_possible:
                        r.possible_values.append(value)
                return r

            def getRowColSquareLists(self, is_clear_down=False):
                last_x0 = -1
                last_y0 = -1
                row_list = []
                col_list = []
                ...
                for row in range(9):
                    ...
                    for col in range(9):
                        r = self[row][col]
                        try:
                            one_col = col_list[col]
                            ok = True
                        except Exception as e:
                            one_col = []
                            col_list.append(one_col)
                        one_col.append(r)
                        r.my_col = one_col
                        one_row.append(r)
                        r.my_row = one_row
                        ...
                        x0 = (row // 3) * 3
                        y0 = (col // 3) * 3
                        # tbl = [((x, y, (x // 3) * 3, (y // 3) * 3)) for x in range(9) for y in range(9)]
                        is_x_changed = (x0 != last_x0)
                        is_y_changed = (y0 != last_y0)
                        is_new_square = (is_y_changed or is_x_changed)
                        if is_new_square or (one_square is None):
                            last_y0 = y0
                            last_x0 = x0
                            try:
                                square_list_flat_index = x0 + (y0 // 3)
                                one_square = square_list[square_list_flat_index]
                                ok = True
                            except Exception as e:
                                one_square = []
                                square_list.append(one_square)

                        is_within_square = (y0 <= col < y0 + 3) or (x0 <= row < x0 + 3)
                        if is_within_square:
                            try:
                                r.my_square = one_square
                                one_square.append(r)
                            except Exception as e:
                                debug = True
            ...
            def genPossibleValueForEachLocation(self):
                for row in range(9):
                    for col in range(9):
                        self.genPossibleValues(row, col)

            def generateGrid(self):
                def addMyRowColSquare():
                    rows, cols, squares = self.getRowColSquareLists()
                ...
                def makeRecord(loc):
                    ...
                    r = LocRecord(grid_value, is_temporal=(grid_value == 0), row=row, col=col)
                    return r

                def makeColumns(row: int):
                    col_range=list(range(9))
                    row_col_list = [(row, col) for col in col_range]
                    one_row = list(map(makeRecord, row_col_list))
                    return one_row

                self.clear()
                row_range=list(range(9))
                all_rows = list(map(makeColumns, row_range))
                self.extend(all_rows)
                addMyRowColSquare()
                self.genPossibleValueForEachLocation()
                debug = True

            def solveEachLocation(self, location):
                row = location[0]
                col = location[1]

            def solve(self):
                location_list = [(row, col) for row in range(9) for col in range(9)]
                list(filter(self.solveEachLocation, location_list))
            # def isPossibleCell(self, entry):
            #     row = entry[0]
            #     col = entry[1]
            #     value = entry[2]
            #     cell_value = grid[row][col]
            ...
            #     return is_taken
            #
            # def isPossibleCol(self, col: int, value: int):
            #     row_range = list(range(9))
            #     entry_list = [(row, col, value) for row in row_range]
            #     is_taken_list = list(filter(self.isPossibleCell, entry_list))
            #     is_taken = any(is_taken_list)
            #     return is_taken
            #
            # def isPossibleRow(self, row: int, value: int):
            #     col_range = list(range(9))
            #     entry_list = [(row, col, value) for col in col_range]
            #     is_taken_list = list(filter(self.isPossibleCell, entry_list))
            #     is_taken = any(is_taken_list)
            #     return is_taken
            #
            # def isPossibleSquare(self, x: int, y: int, value: int):
            #     x0 = (x // 3) * 3
            #     y0 = (y // 3) * 3
            #     for row in range(0, 3):
            #         for col in range(0, 3):
            ...
            #     return [-1, -1, -1]
            #
            # def possible(self, row: int, col: int, value: int):
            #     is_possible = True
            #     is_taken = self.isPossibleRow(row, value)
            #     if is_taken:
            #         return False
            #
            #     is_taken = self.isPossibleCol(col, value)
            #     if is_taken:
            #         return False
            #
            #     (x, y, cell_value) = self.isPossibleSquare(row, col, value)
            #     is_taken = (x != -1)
            #     if is_taken:
            #         cell_value = grid[x][y]
            #         return False
            ...
            #     is_filled = (cell != 0)
            #     if not is_filled:
            #         is_pos = self.possible(row, col, value)
            #         if is_pos:
            #             grid[row][col] = value
            #             changed = (row, col)
            #     return changed
            #
            # def tryRangeOfValues(self, row: int, col: int):
            #     value_entry_list = [(row, col, value) for value in value_range]
            #     changed_list = list(filter(self.tryValue, value_entry_list))
            #     return any(changed_list)
            #
            # def tryRow(self, row: int):
            #     col = self.tryRow.col
            #     changed = self.tryRangeOfValues(row, col)
            #     return changed
            #
            # def tryCol(self, col: int):
            #     row_range = list(range(9))
            #     self.tryRow.col = col
            #     changed_list = list(filter(self.tryRow, row_range))
            #     changed = any(changed_list)
            #     return changed
            #
            # def testValue(self, value: int):
            #     col_range = list(range(9))
            #     self.tryCol.value = value
            #     changed_list = list(filter(self.tryCol, col_range))
            #     changed = any(changed_list)
            #     if changed:
            #         cell_value = grid[0][0]
            #         debug = True
            ...
            #     value_range = deque(list(range(1, 10)))
            #     for i in range(9):
            #         changed_list = list(map(self.testValue, value_range))
            #         is_finished = self.checkValidGrid()
            #         if not is_finished:
            #             pp(grid)
            #             print(f'tried times: {i + 1}, value_range:{value_range}')
            #             self.resetGrid(orig_grid)
            #             value_range.rotate(1)
            #         else:
            #             pp(grid)
            #     pp(orig_grid)
        ...
        value_range = deque(list(range(1, 10)))
        grid_record = GridRecords(grid)
        grid_record.generateGrid()
        pp(grid_record)
        #
        #
        #
    ...
    def tranRefDriver(self):
        ...
        if is_debug:
            ...
            ''', re.VERBOSE)
            # ((?<!\s)[diouxXeEfFgGcrs%])
            t = "Use Inverse square falloff, and a small falloff distance value with a higher light intensity will give the best results with POV-Ray's implementation of inverse square law. See this `discussion <http://news.povray.org/povray.general/thread/%3Cweb.4d77b443f36cbfe281c811d20%40news.povray.org%3E/>`__."
            m = PYTHON_FORMAT.search(t)
            print(m)
            exit(0)
        driver = RefDriver()
    ...
    def cleanGARef(self):
        home = self.home
        input_file = os.path.join(home, 'cor_0015.po')
        output_file = os.path.join(home, 'cor_0016.po')
        tf = TranslationFinder()
        data = c.load_po(input_file)
        for m in data:
            mid = m.id
            pu.patternMatchAll(df.GA_REF, mid)

    def test_match_case(self):
        mid = "ID Mask node example."
        mstr = "Ví dụ: về nút Chắn Lọc Id."
        new_mstr = cm.matchCase(mid, mstr)
        print(new_mstr)

    def run(self):
        # self.test_match_case()
        self.test_pat()
        # exit(0)
        # self.cleanGARef()
        # self.tranRefDriver()
