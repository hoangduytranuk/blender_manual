...
class ParseRST():
   ...
   #HTML Parser Methods
    ...
    def parse_term(self, elem):
        ...
        self.term_list.append(text_data)
    ...
    #def fuzzyWordMatch(self, s1, s2):
        #print("fuzzyWordMatch s1:{}; s2:{}".format(s1, s2))
        ...
        #for s1_w in s1_word_list:
            ...
            #print("s1_w:{}; distance:{}".format(s1_word_list, percentage))
    ...
    #def fuzzySearch(self, search_text_line, possible_list):
        ...
        #for index, entry in enumerate(possible_list):
            #candidate, ratio = entry
            #fuzzy_dist = self.fuzzyWordMatch(search_text_line, candidate)
            #match_list.append((fuzzy_dist, candidate))
        #sorted_list = sorted(match_list, reverse=True)
        #print("fuzzySearch sorted_list:{}".format(sorted_list))
        #if (len(match_list) > 0):
            #fuzzy_dist, candidate = sorted_list[0]
            #print("fuzzySearch fuzzy_dist:[{}], candidate:[{}]".format(fuzzy_dist, candidate))
            #result = [fuzzy_dist, candidate]
        #print("-" * 50)
        #return result
    ...
    def parseOneFile(self):
        ...
        #print(os.linesep.join(self.srt_data_list))

        #common_part = source_path[len(file_route):]
        #print("tag:{}".format(tag['source']))
        #print("source_path:{}".format(self.source_path))
        #print("common_part:{}".format(common_part))
        #exit(0)

        kw = ['title', 'field_list', 'term', 'strong', 'rubric', 'note', 'bullet_list']
        for k in kw:
    ...
    def BuildDictionaryFromRST(self):
